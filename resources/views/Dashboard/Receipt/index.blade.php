...
@section('title')
    {{ trans('Dashboard/receipts.title') }}
@stop
@section('css')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                ...
                <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ trans('Dashboard/receipts.title') }}</span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
                ...
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between">
                        <a href="{{ route('Receipt.create') }}" class="btn btn-primary" role="button"
                           aria-pressed="true">{{ trans('Dashboard/receipts.add_new_receipt') }}</a>
                    </div>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table text-md-nowrap" id="example1">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>{{ trans('Dashboard/receipts.patient_name') }}</th>
                                    <th>{{ trans('Dashboard/receipts.amount') }}</th>
                                    <th>{{ trans('Dashboard/receipts.description') }}</th>
                                    <th>{{ trans('Dashboard/receipts.created_at') }}</th>
                                    <th>{{ trans('Dashboard/receipts.actions') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                   ...
                                   <tr>
                                       ...
                                       <td>
                                           <a href="{{ route('Receipt.edit', $receipt->id) }}" class="btn btn-sm btn-primary">
                                               <i class="fas fa-edit"></i> {{ trans('Dashboard/receipts.edit') }}
                                           </a>
                                           <a class="modal-effect btn btn-sm btn-danger" data-effect="effect-scale"
                                              data-toggle="modal" href="#delete{{ $receipt->id }}">
                                               <i class="las la-trash"></i> {{ trans('Dashboard/receipts.delete') }}
                                           </a>
